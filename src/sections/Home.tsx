...
export default function Home({ onNavigate }: { onNavigate: (t: Tab) => void }) {
  return (
    <section className="mx-auto flex min-h-screen w-full max-w-5xl flex-col justify-center px-4 pb-32 pt-32 sm:px-6">
      ...
      <p className="mt-4 text-xl text-[var(--ink-dim)] sm:text-2xl">
        {profile.title} - <span className="text-white/90">{profile.tagline}</span>
      </p>
      <p className="mt-2 text-sm font-semibold uppercase tracking-[0.3em] text-[var(--ink-faint)]">
        {profile.location}
      </p>
